...
从 0.1.2-alpha.2 起，上游连环回 RPC 也要求认证，但浏览器仍按主机名选择内存持久化。
网关认证租户，隧道通过 Connection 的公开方法取得独立的 DSH 本地会话；浏览器的主机名检查
仍阻止它发送服务端已经允许的写入。配置表达不了它，组合顺序绕不过它，而在插件里翻那个标志又必然
...
一处改动属于哪一个插件，由同一个问题决定：**把网关拿掉，这件事还需要吗？** 现在有七个插件
落在这个问题上：
...
- `dsh-computer` 是 agent 和人共用的浏览器与桌面：只读画面通道、可交互的 noVNC
  界面，以及 agent 在登录、MFA、验证码或同意操作需要人时用的工具/卡片交接。它在拿掉网关后仍然
  成立；只有它渲染进去的 artifact panel 座位属于这套部署布局。
...
一处改动七个都放不进去，说明上面那个问题有了新答案，而不是说明其中某一个该再长出第二个
