 ...
 * The artifact fixture also applies the plugin and exercises its file-open
 * callback against the official docking contract; it does not mount a DOM.
 *
...
import { checkArtifactPanelOpen } from './check-panel-open.mjs'
  ...
  for (const entry of entries) {
    ...
    if (name === 'dsh-artifact-panel') {
      try {
        ...
        problems.push(`${relative}: official docking registration failed — ${error.message}`)
      }
    }
  }
