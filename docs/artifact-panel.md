...
At the normal sidebar width, the desktop is the first thing in the pane: a rounded card with the native **1280:720** aspect. Its width follows the draggable panel width and its height follows from that ratio. noVNC therefore scales the desktop instead of filling a tall column with letterbox above and below it. The artifact panel declares an empty seat; `dsh-computer` follows it and renders the desktop on the panel's token-driven surface, so light, dark and deployment skins remain the source of colour.

The section below is the scheduled-task manager, not a copy of it. `dsh-scheduled-tasks` owns the list, loading, create/edit form, enable/disable and two-step deletion; `dsh-computer` declares its empty data-attribute seat beneath the desktop. The scheduled plugin follows that seat and mounts its own React root there because the panel itself is a separate body-level root. `scripts/check-computer-layout.mjs` holds the panel-to-computer seat, computer-to-schedule seat and 1280:720 aspect together across all three plugins.

`dsh-computer` also owns `computer_request_user_action`. An agent calls it only after navigating as far as it safely can and reaching a human-only step such as login, MFA, CAPTCHA or consent. The running tool row becomes a live-preview card with three actions: **Take over** opens this Computer pane without resolving the wait; **I am done** and **Skip** answer DSH's public `userQuestions` request. The tool then resumes the same turn and defers a plugin-sourced context message carrying the outcome, instead of manufacturing text in the user's composer. The tool description explicitly forbids asking for passwords, verification codes or other secrets in chat.
...
Every frame on the desktop — the seat, the takeover — is one `DesktopFrame`, and it is covered until noVNC's page says it is connected. The page, its modules, the websocket and the RFB handshake all travel the tunnel, and the layer-coloured blank that used to fill that second read as broken. The cover reads the frame's same-origin document for noVNC's own `noVNC_connected` root class, and follows retries and reconnects. It steps aside for noVNC's errors, credentials and connection dialogs, or a page the gateway refused; dismissing an error leaves the retry button visible. The observer reads an already-loaded frame too, so a cached page cannot miss the readiness check. **Open in new window** lands on the same page with no frame around it, so the page carries its own dress: `sandbox/desktop/patch-novnc.sh` gives noVNC's `vnc.html` this deployment's favicon in place of noVNC's, a cover of its own, and a bootstrap that takes the ground, ink tab name and translated loading message from the URL `dsh-computer` wrote. `scripts/check-computer-loading.mjs` exercises the readiness states and bootstrap; `scripts/check-images.sh` checks the installed page and assets. On a desktop deployment, `VERIFY_DESKTOP=1` adds `verify/verify-computer-loading.mjs` to the acceptance run, requiring a real framebuffer connection and exercising the loading and retry controls in a browser.
...
The fourth cannot: the browser watch reads CDP, which listens on the sandbox's own loopback — deliberately, since that port drives the browser as the tenant — and envd does not forward TCP. So that one tab rides the `/browser` channel `dsh-computer` registers through the tunnel. The panel stays a browser-only package either way; the host half belongs to the plugin that owns the shared browser and desktop.
